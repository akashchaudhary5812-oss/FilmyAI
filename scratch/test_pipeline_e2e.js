...
const UploadFilm = require('../Backend/src/models/UploadFilm.model');
const { triggerMLPipeline } = require('../Backend/src/services/mlPipeline.service');
...
async function main() {
    ...
    const pipelinePromise = triggerMLPipeline(filmId, {
        FilmName: film.FilmName,
        DirectorName: film.DirectorName,
        Casting: film.Casting,
        ProductionHouses: film.ProductionHouses.join(', '),
        Budget: film.Budget,
        Genre: film.Genre,
        ReleaseYear: film.ReleaseYear,
        ReleaseMonth: film.ReleaseMonth,
        IsSequel: film.IsSequel,
        Summary: film.Summary,
        Script: film.Script,
        uploadFilm: film.uploadFilm,
        generate_pdf: true
    });
    ...
    while (!completed && attempts < maxAttempts) {
        ...
        try {
            const statusRes = await axios.get(`${BACKEND_URL}/api/film/${filmId}/status`);
    ...
    const reportRes = await axios.get(`${BACKEND_URL}/api/film/${filmId}/report`);
